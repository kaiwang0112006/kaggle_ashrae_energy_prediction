...
def rmsd_score(y_true, y_score):
    score = mean_squared_error(y_true, y_score)
    return score

def prep(df,tp=''):
    df['month'] = df['day'].apply(lambda x:int(x[5:7]))
    df['year'] = df['day'].apply(lambda x: int(x[:4]))
    df['day'] = df['day'].apply(lambda x: int(x[-2:])) # np.expm1
    if tp=='train':
        df['meter_reading'] = df['meter_reading'].apply(lambda x:np.log1p(x))
    df = df.drop(columns=['timestamp'])
    return df

def main():
    train_org = pd.read_csv('../train_bu_weather.csv')
    test = pd.read_csv('../test_bu_weather.csv')

    train_org = prep(train_org,tp='train')
    test_org = prep(test)

    train, test = train_test_split(train_org, test_size=0.33, random_state=42)

    features = [f for f in train if f not in ['index','timestamp','building_id','site_id','meter_reading']]
    ...
    test_org[['row_id','meter_reading']].to_csv('submitv1opt.csv',index=False)
